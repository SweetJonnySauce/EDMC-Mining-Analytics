...
## Vendored Harness Files (Immutable)
- `tests/harness.py` is vendored from upstream and must be treated as immutable.
- Everything under `tests/edmc/` is vendored from upstream and must be treated as immutable.
- Do not edit these files during normal feature work.
- Only update them when explicitly requested to re-vendor/sync from upstream; in that case, copy as-is and document source branch/commit.
...
- **After wiring changes:** rerun the headless suite once per milestone to catch integration regressions.
...
- Confirm target Python version matches the version stated in EDMC core `docs/Releasing` and the plugin README requirements.
